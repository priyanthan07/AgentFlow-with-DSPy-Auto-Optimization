...
class MLflowManager:
    ...
    def start_optimization_run(self, run_name: str, config: Dict[str, Any]):
        ...
        run = mlflow.start_run(run_name=f"Optimization_{run_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def start_evaluation_run(self, run_name: str, agent_type: str):
        ...
        run = mlflow.start_run(run_name=f"Evaluation_{agent_type}_{run_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
    ...
    def load_optimized_agent(self, agent_class, agent_name: str, version: str = "latest"):
